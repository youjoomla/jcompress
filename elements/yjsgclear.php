	...
	public function getInput()
		{
			...
			$menuListLog	= JPATH_ROOT.DIRECTORY_SEPARATOR."cache".DIRECTORY_SEPARATOR."jcompress".DIRECTORY_SEPARATOR.'menuList.php';
			...
			if(JFile::exists($menuListLog)){
				
				include $menuListLog;
				
				$menuItems 	= unserialize($menulist);
				$all_links = array();
				
				foreach($menuItems as $link){
					$all_links[]= "'".$link['full_link']."'";
				}
				
				$links_array = implode(',',$all_links);	
				$document->addScriptDeclaration("var links_array=[".$links_array."];");
			}
			
			
			
			$filescount_css = getFileCount($cache_folder.DIRECTORY_SEPARATOR.'css',array('_css','_log'));
			$filescount_js = getFileCount($cache_folder.DIRECTORY_SEPARATOR.'js',array('_js','_log'));
